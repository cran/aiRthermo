...
function(Pstart,Tstart,wstart,Pend,deltaP=1){
  ...
  outvals<-.C("adiabatic_ascent_Rworld",as.double(Pstart),as.double(Tstart),as.double(wstart),
              as.double(Pend),as.double(Tend),as.double(wend),
              as.double(deltaP))
  
  names(outvals)<-c( "Pstart", "Tstart","wstart","Pend","Tend","wend",
                     "deltaP")
